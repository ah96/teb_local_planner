...
{
		ros::init(argc, argv, "perturb_node_tabular");
		ros::NodeHandle n("~");
		...
		for(int i = 0; i < local_costmap_data.size(); i++)
		{
			for(int j = 0; j < local_costmap_data[0].size(); j++)
			{
				arr[i*160+j] = local_costmap_data[i][j];
			}
		}			
		unsigned char* local_costmap_data_ptr = arr;
		...
		for(int i = 0; i < odom.size(); i++)
		{
			...
			uint32_t rez = wrapper.computeVelocityCommands(odom_pose[i], odom_twist[i], cmd_vel, message); // compute command velocities
			std::cout << "Da li je racunanje komandnih brzina bilo ispravno (0-SUCCESS) = " << rez << std::endl << std::endl;
